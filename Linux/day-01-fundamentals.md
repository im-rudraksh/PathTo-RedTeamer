...
DAY -->1 (LINUX FUNDAMENTALS)
	|--> I am currently using TRYHACKME.COM's Linux fundamental path
	  ...
	  |--> 4) "find" used to find a file(s)/folder(s) i.e. there path starting search from root directory
 	  |       $ find -name "file_name.txt"   //-- here -name is a subcmd of find that specifies to find file using name ,the name of file is in brackets
	  ...
	  |       $ find -name *.txt         //--(don't use quotes) * is a wildcard here its used to find any file with .txt extention in the directory and sub directories
	  |	  $ find /path/to/find -name "example.tx" //---to find along a specified path
	  |	  $ find . -type f   //-- "." helps find inside current directory only and "-type" helps find by its type "f" for files and "d" for directories
	  |       $ find ~ -type d   //-- "~" helps find wintin scope of current user
	  |       $ fidn . -name example.txt -delete  //-- finds and deletes the file
	  |
	  |--> 5) "cat" concatinate --used for showing content of file in terminal without opening the file
	  |       $ cat
	  |
	  ...
	  |       $ grep -o "pattern" filename.txt  //-- return only the word/part that matches the searche pattern
	  ...
	  |      |-> ">>" same as redirefctor but it appends the data instead of replacing 
